...
function getPenName(displayName: string | null | undefined): string {
  return localStorage.getItem('letters_pen_name') || displayName || 'You'
}
...
export function LetterEditorPage() {
  ...
  const savedSnapshot = useRef({ recipient: '', subject: '', content: '', mood: null as string | null, letterDate: todayIso(), sealedUntil: null as string | null })
  ...
  const penName = getPenName(user?.displayName)
  ...
  // Confirm and proceed when blocker fires
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const dirty =
      ...
      mood !== snap.mood ||
      letterDate !== snap.letterDate ||
      sealedUntil !== snap.sealedUntil
    setIsDirty(dirty)
  }, [recipient, subject, content, mood, letterDate, sealedUntil])
    ...
    if (!isNew && id) {
      lettersApi.get(id)
        .then(detail => {
          ...
          const m = detail.mood
          const ld = detail.letterDate ?? todayIso()
          const su = detail.sealedUntil

          setRecipient(r); setSubject(s); setContent(c)
          setMood(m); setLetterDate(ld); setSealedUntil(su)
          if (su) setSealToggle(true)

          savedSnapshot.current = { recipient: r, subject: s, content: c, mood: m, letterDate: ld, sealedUntil: su }
          setIsDirty(false)
        })
  ...
  async function saveLetter() {
    ...
    setSaving(true)
    try {
      const su = sealToggle ? sealedUntil : null
      const payload = {
      ...
      savedSnapshot.current = { recipient: recipient.trim(), subject: subject.trim(), content, mood, letterDate, sealedUntil: su }
      setIsDirty(false)
      setSaved(true)
  ...
  async function exportAsImage() {
    ...
    try {
      ...
      // Replace form elements with plain text nodes for accurate capture
      const inputs = Array.from(paper.querySelectorAll<HTMLElement>('input, textarea'))
      const restores: (() => void)[] = []
      ...
      inputs.forEach(el => {
        ...
        restores.push(() => { el.style.display = ''; div.remove() })
      })

      // Expand paper to full content height
      const prevOverflow = paper.style.overflow
      const prevHeight = paper.style.minHeight
      paper.style.overflow = 'visible'
      paper.style.minHeight = 'auto'

      const dataUrl = await toPng(paper, { cacheBust: true, pixelRatio: 2 })

      // Restore
      paper.style.overflow = prevOverflow
      paper.style.minHeight = prevHeight
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f5f0e8' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {isDirty && <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', marginLeft: 4 }}>unsaved</span>}
        </div>
        <div style={{ display: 'flex', gap: 6 }}>
          {!isNew && !sealed && (
            <button onClick={exportAsImage} disabled={exporting} title="Export as image"
              style={{ background: 'rgba(255,255,255,0.15)', border: 'none', borderRadius: 8, color: '#fff', padding: '6px 10px', cursor: 'pointer', fontSize: 13 }}>
              {exporting ? '…' : '📤'}
            </button>
          )}
          <button onClick={saveLetter} disabled={saving}
            style={{
              ...
              cursor: saving ? 'default' : 'pointer', fontSize: 13, fontWeight: 500, transition: 'background 0.3s',
            }}>
        ...
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          {sealed ? (
            <div ref={paperRef} style={{
              background: '#fffef9', border: '1px solid #e2d5c0', borderRadius: 4,
              ...
              <div style={{ fontFamily: 'Georgia, serif', fontWeight: 600, fontSize: 20, color: '#3d2b1f' }}>Time Capsule</div>
              <div style={{ fontFamily: 'Georgia, serif', fontSize: 14, color: '#9a8060', fontStyle: 'italic' }}>Dear {recipient},</div>
              <div style={{ fontSize: 13, color: '#b8a080' }}>This letter is sealed until</div>
              <div style={{ fontFamily: 'Georgia, serif', fontWeight: 600, fontSize: 16, color: '#5c4a3a' }}>
                {sealedUntil ? formatDateDisplay(sealedUntil) : ''}
              </div>
              <div style={{ fontSize: 12, color: '#c8b898', marginTop: 8 }}>Come back then to read it.</div>
            </div>
          ) : (
            /* Paper editor */
            <div ref={paperRef} style={{
              ...
              padding: '32px 36px 40px', minHeight: 500, position: 'relative',
            }}>
              {/* Red margin line */}
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                <input type="date" value={letterDate} onChange={e => setLetterDate(e.target.value)}
                  style={{ fontFamily: 'Georgia, serif', fontSize: 13, color: '#9a8060', border: 'none', background: 'transparent', outline: 'none', cursor: 'pointer' }} />
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 8 }}>
                ...
                <input value={recipient} onChange={e => setRecipient(e.target.value)} placeholder="…"
                  style={{ fontFamily: 'Georgia, serif', fontSize: 17, color: '#3d2b1f', border: 'none', borderBottom: '1px dashed #c8b090', background: 'transparent', outline: 'none', flex: 1, minWidth: 80 }} />
              ...
              <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="Subject (optional)"
                style={{
                  width: '100%', boxSizing: 'border-box', fontFamily: 'Georgia, serif', fontStyle: 'italic',
                  fontSize: 14, color: '#6b5040', border: 'none', borderBottom: '1px dashed #e0cdb0',
                  background: 'transparent', outline: 'none', marginBottom: 20, paddingBottom: 4,
                }} />
              ...
              <div style={{ position: 'relative' }}>
                ...
                <textarea ref={contentRef} value={content}
                  ...
                  placeholder="Write what you never said…" rows={14}
                  style={{
                    width: '100%', boxSizing: 'border-box', fontFamily: 'Georgia, serif', fontSize: 15.5,
                    lineHeight: `${lineHeight}px`, color: '#2d1f0f', border: 'none', background: 'transparent',
                    outline: 'none', resize: 'none', overflow: 'hidden', position: 'relative', zIndex: 1, padding: 0,
                  }}
                  onInput={e => {
              ...
              {/* Word count */}
              <div style={{ textAlign: 'right', fontSize: 11, color: '#c0b090', marginTop: 8 }}>
                {wordCount} {wordCount === 1 ? 'word' : 'words'}
              </div>

              {/* Sign-off */}
              <div style={{ marginTop: 24, textAlign: 'right', fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 14, color: '#9a8060' }}>
                — {penName}
              </div>
              {letterDate && (
          ...
          <div style={{ marginTop: 16, background: '#fff', border: '1px solid #e8e0d0', borderRadius: 12, padding: '14px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 14, color: '#3d2b1f' }}>🔒 Time capsule</div>
                <div style={{ fontSize: 12, color: '#b0a080', marginTop: 2 }}>Seal this letter until a future date</div>
              </div>
              <button onClick={() => {
                setSealToggle(t => !t)
                if (!sealedUntil) {
            ...
            {sealToggle && (
              <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <input type="date" value={sealedUntil ?? ''} min={todayIso()}
                  onChange={e => setSealedUntil(e.target.value || null)}
                  style={{ fontFamily: 'Georgia, serif', fontSize: 13, color: '#3d2b1f', border: '1px solid #d0c0a0', borderRadius: 6, padding: '4px 8px', background: '#fdf8f0', outline: 'none', cursor: 'pointer' }} />
                {sealed && <span style={{ fontSize: 12, color: '#9a8060', fontStyle: 'italic' }}>· currently sealed</span>}
              </div>
            )}
          </div>
